...
const loadLevelWord = (id) => {
  ...
  fetch(url)
    .then((res) => res.json())
    .then((data) => {
      removeActive(); //all active remove
      const clickBtn = document.getElementById(`lesson-btn-(${id})`);
      clickBtn.classList.add("active"); //only this btn add active
...
const displayLevelWord = (words) => {
  ...
  if (words.length === 0) {
    wordContainer.innerHTML = `
          <div class="text-center rounded-xl col-span-full space-y-6 font-bangla">
